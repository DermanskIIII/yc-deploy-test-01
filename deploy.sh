...
git clone $GITHUB_CLONE_URL/$GITHUB_REPO_NAME.git 2>&1 >/dev/null
...
newgrp docker << EOF
  ...
  docker build -t $DOCKER_IMAGE_NAME . 2>&1 >/dev/null
  echo "Успешно"
  cd ~
  rm -rf $TEMP_DIR
  docker tag $DOCKER_IMAGE_NAME cr.yandex/\$YC_REGISTRY_ID/$DOCKER_IMAGE_NAME:$DOCKER_IMAGE_TAG
  echo "Загружаем образ в реестр..."
  docker push cr.yandex/\$YC_REGISTRY_ID/$DOCKER_IMAGE_NAME:$DOCKER_IMAGE_TAG 2>&1 >/dev/null
  echo "Успешно"
